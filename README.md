...
- **`python/`**: Clean Python implementation with interactive 3D visualisation (see [python/README.md](python/README.md)).
...
A standalone Python implementation is available in the [`python/`](python/) folder with:
- NumPy/SciPy-based functional map pipeline (cotangent Laplacian, ZoomOut, time-consistent refinement)
- Interactive 3D visualisation using [Polyscope](https://polyscope.run/) with live frame-by-frame playback
